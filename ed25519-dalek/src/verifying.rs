...
impl VerifyingKey {
    ...
    /// [`Self::ComputeR`]). `CtxDigest` is the digest used to calculate the pseudorandomness
        ...
        CtxDigest: Digest<OutputSize = U64>,
    {
        ...
        let expected_R = ComputeR::<CtxDigest>::compute(self, signature, None, message);
        ...
        F: Fn(&mut CtxDigest) -> Result<(), SignatureError>,
    {
        ...
        let mut c = ComputeR::<CtxDigest>::new(self, signature, None);
        msg_update(&mut c.h)?;
        ...
        MsgDigest: Digest<OutputSize = U64>,
    {
        ...
        let expected_R = ComputeR::<CtxDigest>::compute(self, signature, Some(ctx), &message);
    ...
    pub fn verify_strict(
        ...
        let expected_R = ComputeR::<Sha512>::compute(self, signature, None, message);
        if expected_R == signature.R {
            Ok(())
        } else {
            Err(InternalError::Verify.into())
        ...
        MsgDigest: Digest<OutputSize = U64>,
    {
        ...
        let message = prehashed_message.finalize();
        let expected_R = ComputeR::<Sha512>::compute(self, signature, Some(ctx), &message);
...
pub(crate) struct ComputeR<CtxDigest> {
    key: VerifyingKey,
    signature: InternalSignature,
    h: CtxDigest,
}
...
#[allow(non_snake_case)]
impl<CtxDigest> ComputeR<CtxDigest>
where
    CtxDigest: Digest<OutputSize = U64>,
{
    pub fn compute(
